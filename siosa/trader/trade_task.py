...
class TradeTask(Task):
    ...
    def __init__(self, trade_info):
        ...
        self.steps = TradeTask.steps(self.info)

    @staticmethod
    def steps(trade_info):
        stash_item = StashItem(
            trade_info.item,
            trade_info.stash_tab,
            trade_info.trade_request.position)
        return [
            PickupItem(stash_item)
            # InvitePlayerToHideoutStep(trade_info.trade_request.trader),
            # IncomingTradeStep(trade_info)
        ]
